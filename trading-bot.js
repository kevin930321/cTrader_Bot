...
class TradingBot {
    ...
    async init() {
        try {
            ...
            this.connection.on("app-auth-success", async () => {
                console.log(
                    "🔄 Application Auth 成功，正在進行 Account Auth...",
                );
                try {
            ...
            try {
                ...
                if (currentSl === undefined || currentTp === undefined) {
                    ...
                    if (
                        ...
                        currentSl = rawToRealPrice(
                            typeof pos.stopLoss === "object" &&
                                pos.stopLoss.toNumber
                                ? pos.stopLoss.toNumber()
                                : pos.stopLoss,
                        );
                    }
                    ...
                    if (
                        ...
                        currentTp = rawToRealPrice(
                            typeof pos.takeProfit === "object" &&
                                pos.takeProfit.toNumber
                                ? pos.takeProfit.toNumber()
                                : pos.takeProfit,
                        );
                    }
                ...
                if (currentSl !== undefined && currentTp !== undefined) {
                    await this.engine.setPositionSlTp(
                        positionId,
                        currentSl,
                        currentTp,
                    );
                    res.json({
                        success: true,
                        message: "Modification command sent",
                    });
                } else {
                    res.status(400).json({
    ...
    bindEvents() {
        ...
        this.engine.on("trade-closed", (trade) => {
            ...
            if (totalTrades > 0 && totalTrades % 10 === 0) {
                ...
                // 計算本期區間統計 (最近 10 次)
                const periodWins =
                    this.engine.wins - this.engine.lastReportWins;
                const periodLosses =
                    this.engine.losses - this.engine.lastReportLosses;
                ...
                this.io.emit("realtime-update", {
                    ...
                    winRate:
                        this.engine.wins + this.engine.losses > 0
                            ? (
                                  (this.engine.wins /
                                      (this.engine.wins + this.engine.losses)) *
                                  100
                              ).toFixed(1) + "%"
                            : "--",
                });
    ...
    getTargetWatchTime() {
        ...
        const marketConfig = isDst
            ? config.market.summer
            : config.market.winter;
    ...
    async checkTime() {
        ...
        const isAfterOpen =
            ...
            (hour === marketConfig.openHour &&
                minute >= marketConfig.openMinute);
        ...
        if (this.lastResetDate !== today) {
            ...
            console.log(
                `📅 新日期: ${today} (${marketStatusStr}, 美股${seasonStr})`,
            );

            // 執行每日重置
            if (this.engine) {
                await this.resetDaily();
                this.lastResetDate = this.engine.lastResetDate || today;

                // 非休市日才嘗試取得基準點
                if (!isWeekend) {
                    console.log("🔄 新交易日，嘗試取得今日基準點...");
                    await this.engine.fetchAndSetOpenPrice();
                }
            }
        }
...
if (DASHBOARD_PASS) {
    ...
    console.warn(
        "⚠️ Dashboard 未設定密碼，建議設定 DASHBOARD_PASSWORD 環境變數",
    );
}
