...
class CTraderConnection extends EventEmitter {
  ...
  async connect() {
    ...
    // 確保每次嘗試連線前先徹底清理舊連線
    this.disconnect({ reconnect: false, clearReconnectTimer: true });
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.on("close", () => {
        ...
        if (this.pendingRequests.size > 0) {
          const pendingError = new Error("Socket closed");
          for (const {
            reject: pendingReject,
          } of this.pendingRequests.values()) {
            pendingReject(pendingError);
          }
      ...
      this.connectTimeout = setTimeout(() => {
        ...
        const timeoutError = new Error("連線逾時");
        this.isConnecting = false;
        this.shouldReconnect = true;
  ...
  handleMessage(message) {
    ...
    switch (payloadTypeName) {
      ...
      case "ProtoOAErrorRes":
        ...
        console.error(
          `❌ API 錯誤: Code=${errorPayload.errorCode}, Desc=${errorPayload.description || "無描述"}, Maintenance=${errorPayload.maintenanceEndTimestamp || "N/A"}`,
        );
        ...
        if (
          ...
          errorPayload.description.includes("not authorized")
        ) {
          console.log("🔄 偵測到授權錯誤，將進入重連流程...");
          this.authenticated = false;
          this.disconnect({ reconnect: true, clearReconnectTimer: true });
        }
        break;
  ...
  /** 斷開連線 */
  disconnect(options = {}) {
    const { reconnect = false, clearReconnectTimer = true } = options;
    ...
    if (clearReconnectTimer && this.reconnectTimeout) {
  ...
  getPayloadTypeId(typeName) {
    ...
    if (typeName.startsWith("ProtoOA")) {
      ...
      if (result === undefined) {
        ...
        console.error(
          `❌ 無法找到 enum 值，可用的值:`,
          Object.keys(enumType.values).filter((k) => k.includes("AMEND")),
        );
      }
      return result || 0;
    } else {
