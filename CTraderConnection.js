...
class CTraderConnection extends EventEmitter {
    ...
    async loadProto() {
        try {
            ...
            console.error(
                "請確認 proto/ 目錄下存在 OpenApiCommonMessages.proto 與 OpenApiMessages.proto",
            );
            throw error;
        }
    }
    ...
    async connect() {
        ...
        // 確保每次嘗試連線前先徹底清理舊連線，但避免觸發主動斷線後的重連
        ...
        return new Promise((resolve, reject) => {
            ...
            console.log(
                `📡 正在連接 cTrader ${this.config.ctrader.mode} 伺服器...`,
            );
    ...
    async sendApplicationAuth() {
        const ProtoOAApplicationAuthReq = this.proto.lookupType(
    ...
    async sendAccountAuth() {
        ...
        const ProtoOAAccountAuthReq = this.proto.lookupType(
    ...
    async send(payloadType, payload) {
        ...
        return new Promise((resolve, reject) => {
            this.pendingRequests.set(clientMsgId.toString(), {
                resolve,
                reject,
                type: payloadType,
            });
    ...
    handleIncomingData(data) {
        ...
        while (true) {
            ...
            if (messageLength > 1024 * 1024 || messageLength < 0) {
                console.error(
                    `❌ 收到異常封包長度: ${messageLength}，清除緩衝區以防止崩潰`,
                );
                this.incomingBuffer = Buffer.alloc(0);
                return;
            }
            ...
            // 提取完整訊息
            const messageBytes = this.incomingBuffer.subarray(
                offset + 4,
    ...
    handleMessage(message) {
        ...
        const pendingRequest =
            message.clientMsgId && this.pendingRequests.has(message.clientMsgId)
                ? this.pendingRequests.get(message.clientMsgId)
                : null;

        if (payloadTypeName === "ProtoOAErrorRes" && pendingRequest) {
            ...
            const error = new Error(
                errorPayload.description ||
                    `API Error: ${errorPayload.errorCode || "UNKNOWN"}`,
            );
        ...
        switch (payloadTypeName) {
            ...
            case "ProtoOAErrorRes": {
                const apiErrorRes = this.proto.lookupType("ProtoOAErrorRes");
                const apiErrorPayload = apiErrorRes.decode(message.payload);
                console.error(
                    `❌ API 錯誤: Code=${apiErrorPayload.errorCode}, Desc=${apiErrorPayload.description || "無描述"}, Maintenance=${apiErrorPayload.maintenanceEndTimestamp || "N/A"}`,
                );
                console.error(`   詳細: ${JSON.stringify(apiErrorPayload)}`);
                this.emit("api-error", apiErrorPayload);

                // 自動重連機制：當偵測到帳戶未授權錯誤時，統一進入排程重連流程
                if (
                    apiErrorPayload.description &&
                    apiErrorPayload.description.includes("not authorized")
                ) {
            ...
            case "ProtoOAOrderErrorEvent":
                const OrderErrorEvent = this.proto.lookupType(
                    "ProtoOAOrderErrorEvent",
                );
                const orderError = OrderErrorEvent.decode(message.payload);
                console.error(
                    `❌ 訂單錯誤: ${orderError.errorCode} - ${orderError.description || "無描述"}`,
                );
    ...
    startHeartbeat() {
        ...
        this.heartbeatInterval = setInterval(() => {
            ...
            try {
                ...
                if (this.proto) {
                    const ProtoHeartbeatEvent = this.proto.lookupType(
                        "ProtoHeartbeatEvent",
                    );
                    const message = ProtoHeartbeatEvent.create({
                        payloadType: 51,
                    });
                    this.send("ProtoHeartbeatEvent", message).catch((err) =>
                        console.debug("Heartbeat 發送忽略:", err.message),
                    );
                }
            } catch (error) {
    ...
    scheduleReconnect() {
        ...
        console.log(
            `🔄 將在 ${delay}ms 後重連 (第 ${this.reconnectAttempts} 次嘗試)...`,
        );
    ...
    disconnect(intentional = true) {
        ...
        if (this.reconnectTimeout && intentional) {
    ...
    getPayloadTypeId(typeName) {
        ...
        if (typeName.startsWith("ProtoOA")) {
            ...
            if (result === undefined) {
                console.warn(
                    `⚠️ [getPayloadTypeId] 找不到 payloadType: ${key} (from ${typeName})`,
                );
                // 嘗試其他可能的命名格式
                ...
                console.error(
                    `❌ 無法找到 enum 值，可用的值:`,
                    Object.keys(enumType.values).filter((k) =>
                        k.includes("AMEND"),
                    ),
                );
            }
    ...
    getPayloadTypeName(typeId) {
        ...
        if (typeId < 2000) {
            ...
            for (const [name, id] of Object.entries(ProtoPayloadType.values)) {
                if (id === typeId) {
                    ...
                    return name
                        ...
                        .join("")
                        .replace("Req", "Req")
                        .replace("Res", "Res") // Already capitalized
            ...
            const ProtoOAPayloadType =
                this.proto.lookupEnum("ProtoOAPayloadType");
            for (const [name, id] of Object.entries(
                ProtoOAPayloadType.values,
            )) {
                if (id === typeId) {
