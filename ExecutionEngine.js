...
class ExecutionEngine extends EventEmitter {
    ...
    getTimeZoneParts(timeZone, date = new Date()) {
    ...
    normalizePriceToReal(price) {
        const numericPrice = convertLongValue(price);
        if (
            numericPrice === null ||
        ...
        return Math.abs(numericPrice) >= API_PRICE_MULTIPLIER
            ? rawToRealPrice(numericPrice)
            : numericPrice;
    }

    normalizeVolume(volume, symbolData) {
        const minVolume = convertLongValue(symbolData?.minVolume) || 1;
        const stepVolume = convertLongValue(symbolData?.stepVolume) || 1;

        let normalized = Math.round(Number(volume) || 0);
        ...
        if (stepVolume > 1) {
            normalized = Math.ceil(normalized / stepVolume) * stepVolume;
        }
    ...
    async initialize() {
        try {
            ...
            if (state) {
                ...
                if (state.config) {
                    this.entryOffset =
                        state.config.entryOffset || this.entryOffset;
                    ...
                    if (state.config.baselineOffsetMinutes !== undefined)
                        this.baselineOffsetMinutes =
                            state.config.baselineOffsetMinutes;
    ...
    async reconcilePositions() {
        try {
            ...
            this.positions = positions.map((p) => {
                ...
                const openTimestamp = convertLongValue(
                ...
                return {
                    ...
                    entryPrice: this.normalizePriceToReal(rawPrice),
                    entryPriceRaw: rawPrice,
                    volume: volumeInLots,
            ...
            if (this.positions.length > 0) {
                ...
                console.log(
                    `🕒 當前會話起始時間: ${sessionOpen.toLocaleString()}`,
                );
    ...
    async getAccountInfo() {
        ...
        try {
            ...
            const traderPayload = ProtoOATraderRes.decode(
                traderResponse.payload,
            );
    ...
    async subscribeToMarketData() {
        try {
            ...
            const symbolData = await this.getSymbolInfo(
                this.config.market.symbol,
            );
            if (!symbolData) {
    ...
    async getSymbolInfo(symbolName) {
        ...
        try {
            ...
            // 尋找匹配的 Symbol
            let symbol = payload.symbol.find(
            ...
            if (!symbol) {
                ...
                for (const candidate of candidates) {
                    ...
                    if (symbol) {
                        console.log(
                            `✅ 自動匹配到替代 Symbol: ${symbol.symbolName}`,
                        );
                        break;
                    }
                }
    ...
    calculateRealTimeAccountInfo() {
        const balance = this.cachedAccountInfo?.balance ?? this.balance ?? 0;
        let unrealizedPnL = 0;

        // 計算每個持倉的即時損益
        const positionsWithPnL = this.positions.map((pos) => {
            const entryPrice = this.normalizePriceToReal(
                pos.entryPriceRaw ?? pos.entryPrice,
            );
            const currentPrice = this.currentPrice
                ? this.normalizePriceToReal(this.currentPrice)
                : null;
            const volume = pos.volume;

            let pnl = null;
            if (currentPrice !== null && entryPrice !== null && volume) {
                if (pos.type === "long") {
        ...
        const usedMargin = this.cachedAccountInfo?.usedMargin || 0;
        const equity = balance + unrealizedPnL;
    ...
    handleExecutionEvent(payload) {
        ...
        if (execType === 3 || execType === "ORDER_FILLED") {
            ...
            if (execution.deal) {
                ...
                if (deal.closePositionDetail) {
                    ...
                    const pendingSlTp = this.pendingSlTp;
                    this.pendingSlTp = null;

                    if (pendingSlTp && execution.position) {
                        const positionId = convertLongValue(
                            execution.position.positionId,
                        );
                        console.log(
                            `📝 正在設定 SL/TP for position ${positionId}...`,
                        );
                        ...
                        ).finally(() => this.reconcilePositions());
                    } else {
                        if (!execution.position) {
                            console.warn(
                                "⚠️ 無法設定 SL/TP: execution.position 不存在",
                            );
                        } else {
                            console.warn(
                                "⚠️ 無法設定 SL/TP: pendingSlTp 不存在",
                            );
                        }
        ...
        // 處理訂單被拒 (例如：保證金不足、市場關閉) - executionType = 4 (ORDER_REJECTED)
            ...
            if (this.orderFailureCount <= 3 && this.isWithinTradingHours()) {
                this.todayTradeDone = false;
                this.isWatching = true;
                this.saveState();
                console.log(
                    `🔄 保留盯盤狀態 (失敗次數: ${this.orderFailureCount}/3)，等待下一次訊號...`,
                );
            } else {
                this.isWatching = false;
                console.error(
                    "⛔ 訂單連續失敗超過 3 次或已超出交易時段，停止今日自動重試。",
                );
            }
    ...
    handleTradeClosed(deal) {
        ...
        const tradeRecord = {
            ...
            type:
                deal.tradeSide === 1 || deal.tradeSide === "BUY"
                    ? "long"
                    : "short", // 1=BUY, 2=SELL
        };
    ...
    isWithinTradingHours() {
        ...
        const marketConfig = this.getMarketConfig(now);
        const openMinutes =
            marketConfig.openHour * 60 + marketConfig.openMinute;
        const closeMinutes = openMinutes >= 7 * 60 ? 6 * 60 : 5 * 60;
    ...
    async openPosition(type) {
        ...
        try {
            ...
            // cTrader Volume 計算：
            // - 先使用既有 lots * 100 邏輯
            // - 再依 symbol 的 minVolume / stepVolume 對齊，避免不同 broker 規格問題
            const requestedVolume = Math.round(this.lotSize * 100);
            const volume = this.normalizeVolume(requestedVolume, symbolData);

            console.log(
                `📊 下單量: ${this.lotSize} lots -> 請求 ${requestedVolume}, 實際送出 ${volume} volume units`,
            );

            // 計算基於基準點的 TP/SL 絕對價格
            // 策略：TP/SL 是相對於「基準點」而非「成交價」
            const openPriceReal = this.normalizePriceToReal(
                this.todayOpenPrice,
            );
            ...
            const currentPriceReal = this.normalizePriceToReal(
                this.currentPrice,
            );
            console.log(
            ...
            if (orderSubmitted) {
                this.isWatching = false;
                console.log("🔒 盯盤狀態已關閉（訂單已送出）");
            } else {
                this.isWatching =
                    this.isWithinTradingHours() && !this.todayTradeDone;
                console.log(
                    `👀 開倉未送出，盯盤狀態維持為: ${this.isWatching ? "開啟" : "關閉"}`,
                );
            }
        }
    }
    ...
    async fetchDailyOpenPrice() {
        ...
        try {
            ...
            const now = new Date();
            const marketConfig = this.getMarketConfig(now);
            const nowTaipei = this.getTaipeiNow();
            ...
            if (nowTaipei < openTimeTaipei) {
                console.warn(
                    "⚠️ 當前時間早於今日開盤時間，嘗試獲取昨日基準點...",
                );
                openTimeTaipei.setDate(openTimeTaipei.getDate() - 1);
            }

            // 台北時間固定為 UTC+8，轉成 UTC timestamp 供 API 使用
            const openTimeUtc = Date.UTC(
                openTimeTaipei.getFullYear(),
                openTimeTaipei.getMonth(),
                openTimeTaipei.getDate(),
                marketConfig.openHour - 8,
                marketConfig.openMinute,
                0,
                0,
            );
            ...
            const baselineTimeTaipei = new Date(
                baselineTimeUtc + 8 * 60 * 60 * 1000,
            );
            ...
            if (payload.trendbar && payload.trendbar.length > 0) {
                ...
                if (targetBar) {
                    ...
                    const barTimeTaipei = new Date(barTimeUtc).toLocaleString(
                        "zh-TW",
                        { timeZone: "Asia/Taipei" },
                    );
                    console.log(
    ...
    async checkMarketStatus() {
        try {
            ...
            if (holidayCheck.isHoliday) {
                return {
                    isOpen: false,
                    reason: `假日: ${holidayCheck.holidayName}`,
                };
            }
    ...
    checkHoliday(holidays, timezone, now) {
        ...
        const current = this.getTimeZoneParts(zone, now);
        const todayDays = Math.floor(
            Date.UTC(current.year, current.month - 1, current.day) / 86400000,
        );
        ...
        for (const holiday of holidays) {
            ...
            if (holiday.isRecurring) {
                ...
                const holidayMonth = holidayDate.getUTCMonth() + 1;
                const holidayDay = holidayDate.getUTCDate();

                if (
                    current.month === holidayMonth &&
                    current.day === holidayDay
                ) {
                    return { isHoliday: true, holidayName: holiday.name };
                }
            }
        ...
        return {
            ...
            winRate:
                this.wins + this.losses > 0
                    ? ((this.wins / (this.wins + this.losses)) * 100).toFixed(
                          1,
                      ) + "%"
                    : "--",
            ...
            symbolInfo: this.symbolInfoCache[this.config.market.symbol]
                ? {
                      name: this.symbolInfoCache[this.config.market.symbol]
                          .symbolName,
                      holidays:
                          this.symbolInfoCache[this.config.market.symbol]
                              .holidays?.length || 0,
                      schedules:
                          this.symbolInfoCache[this.config.market.symbol]
                              .schedule?.length || 0,
    ...
    updateConfig(newConfig) {
        ...
        if (newConfig.baselineOffsetMinutes !== undefined)
            this.baselineOffsetMinutes = parseInt(
                newConfig.baselineOffsetMinutes,
            );
        ...
        try {
            ...
            for (const position of positions) {
                try {
                    ...
                    const rawVolume =
                        position.tradeData?.volume ?? position.volume;
                    const volume =
                    ...
                    const ProtoOAClosePositionReq =
                        this.connection.proto.lookupType(
                            "ProtoOAClosePositionReq",
                        );
                    const message = ProtoOAClosePositionReq.create({
                        ctidTraderAccountId: parseInt(
                            this.config.ctrader.accountId,
                        ),
    ...
    async setPositionSlTp(positionId, stopLoss, takeProfit) {
        try {
            const normalizedStopLoss = this.normalizePriceToReal(stopLoss);
            const normalizedTakeProfit = this.normalizePriceToReal(takeProfit);

            if (normalizedStopLoss === null || normalizedTakeProfit === null) {
                throw new Error("SL/TP 價格無效");
            }

            const ProtoOAAmendPositionSLTPReq =
                this.connection.proto.lookupType("ProtoOAAmendPositionSLTPReq");
            ...
            console.log(
                `✅ SL/TP 已設定: TP=${normalizedTakeProfit.toFixed(2)}, SL=${normalizedStopLoss.toFixed(2)}`,
            );
        } catch (error) {
